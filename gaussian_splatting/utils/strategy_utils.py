...
def split(
    ...
    scales = torch.exp(params["scale"][sel])
...
@torch.no_grad()
def reset_opa_masked(
    params: Union[Dict[str, torch.nn.Parameter], torch.nn.ParameterDict],
    optimizers: Dict[str, torch.optim.Optimizer],
    mask: Tensor,
    value: Tensor,
):
    """Inplace reset the opacity for masked gaussians only.

    Args:
        params: A dictionary of parameters.
        optimizers: A dictionary of optimizers, each corresponding to a parameter.
        mask: Boolean mask indicating which gaussians to reset (True = reset)
        value: The value to clamp opacity to (in logit space)
    """
    # In-place clamp opacities for masked gaussians
    params["opacity"].data[mask] = torch.clamp(
        params["opacity"][mask], 
        max=value
    )
    
    # Reset optimizer states for masked gaussians
    optimizer = optimizers["opacity"]
...
@torch.no_grad()
def duplicate_monogs(
    params: Union[Dict[str, torch.nn.Parameter], torch.nn.ParameterDict],
    optimizers: Dict[str, torch.optim.Optimizer],
    state: Dict[str, Tensor],
    mask: Tensor,
):
    """Inplace duplicate the Gaussian with the given mask (MonoGS version).

    Args:
        params: A dictionary of parameters with MonoGS naming (xyz, scaling, rotation, opacity, f_dc, f_rest).
        optimizers: A dictionary of optimizers, each corresponding to a parameter.
...
def split_monogs(
    ...
    """Inplace split the Gaussian with the given mask (MonoGS version).
    ...
    # Get activated scaling and rotation
    scales = torch.exp(params["scaling"][sel])  # (M, 3)
    quats = F.normalize(params["rotation"][sel], dim=-1)  # (M, 4)
    rotmats = normalized_quat_to_rotmat(quats)  # (M, 3, 3)
    ...
    def param_fn(name: str, p: Tensor) -> Tensor:
        repeats = [N] + [1] * (p.dim() - 1)
        if name == "xyz":
            # Split positions with offset
            p_split = (p[sel].unsqueeze(0) + samples).reshape(-1, 3)  # (N*M, 3)
        elif name == "scaling":
            # Reduce scale
            p_split = torch.log(scales / (0.8 * N)).repeat(repeats)  # (N*M, 3)
        else:
...
def relocate(
    ...
    """
    # support "opacity" with shape [N,] or [N, 1]
    ...
    new_opacities, new_scales = compute_relocation(
        ...
        scales=torch.exp(params["scale"])[sampled_idxs],
...
def inject_noise_to_position(
    ...
    opacities = torch.sigmoid(params["opacity"].flatten())
    scales = torch.exp(params["scale"])
