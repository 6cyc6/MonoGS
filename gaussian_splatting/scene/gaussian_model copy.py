...
class GaussianModel:
    ...
    # =================== Gaussian Model Auxiliary Functions =================== #
    ...
    def create_pcd_from_image(self, cam_info, init=False, scale=2.0, depthmap=None):
        """ Create point cloud from image and depthmap """
        cam = cam_info
        # exposure correction
        image_ab = (torch.exp(cam.exposure_a)) * cam.original_image + cam.exposure_b
        # image_ab = torch.clamp(image_ab, 0.0, 1.0)
        image_ab.clamp_(0.0, 1.0)
        rgb_raw = (image_ab.mul(255)).to(torch.uint8).permute(1, 2, 0).contiguous().cpu().numpy()
        ...
        if depthmap is not None:
            ...
            if depth_raw is None:
                depth_raw = np.zeros((cam.image_height, cam.image_width), dtype=np.float32)
            ...
            if self.config["Dataset"]["sensor_type"] == "monocular":
                ...
                #     * 0.05
                # ) * scale
                noise = (np.random.randn(*depth_raw.shape).astype(np.float32) - 0.5) * 0.05
                depth_raw = (1.0 + noise) * scale
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        # use Open3D to create point cloud
    ...
    def init_lr(self, spatial_lr_scale):
        """ Initialize spatial learning rate scale """
        # for mean and scaling
    ...
    def extend_from_pcd(
        ...
        """ Extend Gaussian Model from point cloud """
        # new 3DGS parameters
        new_xyz = nn.Parameter(fused_point_cloud.detach(), requires_grad=True) # (N, 3)
        new_features_dc = nn.Parameter(
            features[:, :, 0:1].transpose(1, 2).contiguous().detach(), requires_grad=True
        ) # (N, 1, 3)
        new_features_rest = nn.Parameter(
            features[:, :, 1:].transpose(1, 2).contiguous().detach(), requires_grad=True
        ) # (N, F-1, 3) if sh_degree = 0, (N, 0, 3)
        new_scaling = nn.Parameter(scales.detach(), requires_grad=True) # (N, 3)
        new_rotation = nn.Parameter(rots.detach(), requires_grad=True) # (N, 4)
        new_opacity = nn.Parameter(opacities.detach(), requires_grad=True) # (N, 1)
        
        # new unique keyframe IDs and number of observations
        new_unique_kfIDs = torch.ones((new_xyz.shape[0]), dtype=torch.int) * kf_id # (N,)
        new_n_obs = torch.zeros((new_xyz.shape[0]), dtype=torch.int) # (N,)
    ...
    def extend_from_pcd_seq(
        ...
        """ Extend Gaussian Model from point cloud created from image and depthmap """
    ...
    def construct_list_of_attributes(self):
        """ Construct list of attributes for PLY saving """
        l = ["x", "y", "z", "nx", "ny", "nz"]
    ...
    def _prune_optimizer(self, mask):
        """ Prune optimizer tensors based on valid points mask """
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            stored_state = self.optimizer.state.get(group["params"][0], None)
    ...
    def prune_points(self, mask):
        """ Prune points based on prune points mask """
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        """ Concatenate new tensors to existing optimizer tensors """
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
