...
class MonocularDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        # image = torch.from_numpy(image).permute(2, 0, 1).to(device=self.device, dtype=self.dtype).div_(255.0).clamp_(0.0, 1.0)
        image = torch.from_numpy(image).permute(2, 0, 1).contiguous()
        image = image.to(device=self.device, dtype=self.dtype).div_(255.0).clamp_(0.0, 1.0)
